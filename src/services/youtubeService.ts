...
class YouTubeService {
  ...
  async getSeoInsights(title: string): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/seo-insights`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to fetch SEO insights');
      }
  ...
  async downloadVideo(url: string, quality: string, type: 'video' | 'audio', format: string): Promise<DownloadResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/download`, {
        ...
        body: JSON.stringify({ url, quality, type, format }),
      });
